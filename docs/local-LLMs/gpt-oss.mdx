...
This guide will walk you through setting up and using OpenAI's GPT-OSS model with LM Studio and eGovernmentOS.
...
## Step 2: Configure eGovernmentOS to use LM Studio

<Steps>
  <Step title="Add Provider">
    Navigate to `chrome://settings/mukira` and click **Add Provider**.

    <Frame>
      <img src="/images/using-lm-studio-openais-gpt-oss/lm-studio-openais-step6.png" alt="Configure eGovernmentOS to use LMStudio" />
    </Frame>
  </Step>
...
<Cards>
  ...
  <Card title="eGovernmentOS Settings" icon="browser">
...
<Accordion title="Context length errors">
  - Make sure the context length in eGovernmentOS matches LM Studio
