...
async function connectToCdp(
  ...
  try {
    ...
    const connectPromise = ensureBrowserConnected(`http://127.0.0.1:${cdpPort}`)
    const timeoutPromise = new Promise<null>((resolve) => {
      setTimeout(() => {
        logger.warn(
          `CDP connection timed out after 5000ms at http://127.0.0.1:${cdpPort}`,
        )
        resolve(null)
      }, 5000)
    })
...
function configureLogDirectory(logDirCandidate: string): void {
  ...
  try {
    ...
    console.warn(
      `Failed to configure log directory ${resolvedDir}: ${error instanceof Error ? error.message : String(error)
      }`,
    )
  }
}
