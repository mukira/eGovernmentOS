...
export function createHttpServer(config: HttpServerConfig) {
  ...
  const app = new Hono<Env>()
    ...
    .get('/browseros-config', (c) => {
      // Serve default config to prevent 404s
      return c.json({
        providers: [
          {
            name: 'default',
            model: 'gemini-2.0-flash',
            apiKey: process.env.GEMINI_API_KEY || 'configured-in-backend',
            dailyRateLimit: 50,
            providerType: 'google',
            baseUrl: 'https://generativelanguage.googleapis.com', // Satisfy required BaseURL check
          },
        ],
      })
    })
