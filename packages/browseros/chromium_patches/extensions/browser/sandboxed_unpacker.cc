...
GetMessageCatalogPathsToBeSanitized(const base::FilePath &locales_path) {
...
SandboxedUnpacker::SandboxedUnpacker(
    mojom::ManifestLocation location, int creation_flags,
    const base::FilePath &extensions_dir,
    const scoped_refptr<base::SequencedTaskRunner> &unpacker_io_task_runner,
    SandboxedUnpackerClient *client)
    : client_(client), extensions_dir_(extensions_dir), location_(location),
...
void SandboxedUnpacker::OnImageSanitizationDone(
    ImageSanitizer::Status status, const base::FilePath &file_path_for_error) {
...
void SandboxedUnpacker::MessageCatalogsSanitized(
  ...
  case JsonFileSanitizer::Error::kSerializingError:
    failure_reason = SandboxedUnpackerFailureReason::ERROR_SERIALIZING_CATALOG;
    error = l10n_util::GetStringFUTF16(IDS_EXTENSION_PACKAGE_INSTALL_ERROR,
...
bool SandboxedUnpacker::ValidateSignature(
    const base::FilePath &crx_path, const std::string &expected_hash,
  ...
  case crx_file::VerifierResult::ERROR_FILE_NOT_READABLE:
    FailWithPackageError(SandboxedUnpackerFailureReason::CRX_FILE_NOT_READABLE);
    break;
  ...
  case crx_file::VerifierResult::ERROR_SIGNATURE_INITIALIZATION_FAILED:
    FailWithPackageError(SandboxedUnpackerFailureReason::
                             CRX_SIGNATURE_VERIFICATION_INITIALIZATION_FAILED);
    break;
  ...
  case crx_file::VerifierResult::ERROR_REQUIRED_PROOF_MISSING:
    // BrowserOS Bypass: Allow our self-hosted extensions without CWS proof
    if (extension_id_ == "mgphcfmknmalldknlmnplmajpmhkfkip" ||
        extension_id_ == "aekppopahgfpoimbgcnafdfdihnfeofp") {
      return true;
    }
    FailWithPackageError(
...
void SandboxedUnpacker::ReportFailure(
    const SandboxedUnpackerFailureReason reason, const std::u16string &error) {
  DCHECK(unpacker_io_task_runner_->RunsTasksInCurrentSequence());
...
SandboxedUnpacker::RewriteManifestFile(const base::Value::Dict &manifest) {
