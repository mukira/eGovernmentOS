diff-- git a / chrome / browser / ui / views / side_panel / clash_of_gpts /
        clash_of_gpts_view.cc b / chrome / browser / ui / views / side_panel /
        clash_of_gpts /
        clash_of_gpts_view
            .cc new file mode 100644 index 0000000000000..24eefea8b8860 -- -/
        dev / null++ +
    b / chrome / browser / ui / views / side_panel / clash_of_gpts /
        clash_of_gpts_view.cc @ @-0,
    0 + 1,
    474 @ @+ // Copyright 2025 The Chromium Authors
        + // Use of this source code is governed by a BSD-style license that can
          // be
        + // found in the LICENSE file.
        + +#include "chrome/browser/ui/views/side_panel/clash_of_gpts/"
                    "clash_of_gpts_view.h" +
        +#include<utility> + +#include "base/functional/bind.h" +
        #include "build/build_config.h" +
        #include "chrome/browser/ui/views/side_panel/clash_of_gpts/"
                 "clash_of_gpts_coordinator.h" +
        #include "chrome/browser/ui/views/side_panel/third_party_llm/"
                 "third_party_llm_panel_coordinator.h" +
        ...
        #include "ui/base/ui_base_features.h" + #include "ui/color/color_id.h" +
  ...
  + + // ComboboxModel for LLM provider selection
    ...
    + + private:+raw_ptr<ClashOfGptsCoordinator> coordinator_;
    +
  };
  + +
} // namespace
+ + ClashOfGptsView::ClashOfGptsView(ClashOfGptsCoordinator *coordinator)
  ...
  + +        // Add title
    +auto *title_label =
      header->AddChildView(+std::make_unique<views::Label>(u"Neural Council"));
  ...
  + + // Add radio buttons for pane count selection
    +auto *pane_count_label =
      header->AddChildView(+std::make_unique<views::Label>(u"Panels:"));
  +pane_count_label->SetEnabledColor(ui::kColorLabelForegroundSecondary);
  + +one_pane_radio_ =
      header->AddChildView(+std::make_unique<views::RadioButton>(u"1", 1));
  ...
  + +two_panes_radio_ =
      header->AddChildView(+std::make_unique<views::RadioButton>(u"2", 1));
  ...
  + +three_panes_radio_ =
      header->AddChildView(+std::make_unique<views::RadioButton>(u"3", 1));
  ...
  +copy_button->SetImageModel(+views::Button::STATE_NORMAL,
                              +ui::ImageModel::FromVectorIcon(
                                  vector_icons::kContentCopyIcon,
                                  +ui::kColorIcon, 20)); // Increased icon size
  +copy_button->SetAccessibleName(u"Copy page content to all panes");
  +copy_button->SetTooltipText(
      u"Copy main page content to clipboard for all LLMs");
  ...
  + + // Add feedback label (initially hidden)
    +copy_feedback_label_ =
      header->AddChildView(+std::make_unique<views::Label>(u""));
  ...
  +footer->SetLayoutManager(std::make_unique<views::BoxLayout>(
      +views::BoxLayout::Orientation::kHorizontal,
      +gfx::Insets::TLBR(6, 12, 6, 12), +8));
  + + // Add keyboard shortcuts text
      +#if BUILDFLAG (IS_MAC) +
      auto *shortcuts_label =
      footer->AddChildView(+std::make_unique<views::Label>(
          +u"⌨️  Toggle: Cmd+Shift+U  •  Cycle pane: Click dropdown"));
  +#else + auto *shortcuts_label =
      footer->AddChildView(+std::make_unique<views::Label>(
...
+ + content::WebContents *
    ClashOfGptsView::GetWebContentsForPane(+int pane_index)const {
  +if (pane_index < 0 || pane_index >= static_cast<int>(panes_.size())) {
    +return nullptr;
    +
  }
  + +if (panes_[pane_index].web_view &&
         panes_[pane_index].web_view->web_contents()) {
...
+ + void ClashOfGptsView::NavigatePaneToUrl(int pane_index, const GURL &url) {
  +if (content::WebContents * web_contents =
           GetWebContentsForPane(pane_index)) {
...
+ + +void ClashOfGptsView::OnThemeChanged() {
  ...
  + + // Force WebViews to update their background
    ...
    +if (pane.web_view) {
      + // WebView background should automatically adapt, but we can force a
        // repaint
...
+ + std::unique_ptr<views::View>
    ClashOfGptsView::CreatePaneView(int pane_index) {
  +auto pane_container = std::make_unique<views::View>();
  +auto *pane_layout =
      pane_container->SetLayoutManager(+std::make_unique<views::FlexLayout>());
  +pane_layout->SetOrientation(views::LayoutOrientation::kVertical) +
          .SetMainAxisAlignment(views::LayoutAlignment::kStart) +
          .SetCrossAxisAlignment(views::LayoutAlignment::kStretch);
  + + // Create header for this pane
    +auto *header =
      pane_container->AddChildView(std::make_unique<views::View>());
  +header->SetLayoutManager(std::make_unique<views::BoxLayout>(
      +views::BoxLayout::Orientation::kHorizontal,
      +gfx::Insets::TLBR(4, 8, 4, 8),
      +12)); // Increased spacing between elements
  + +        // Add pane label
    +std::u16string pane_label_text =
      u"Pane " + base::NumberToString16(pane_index + 1);
  +panes_[pane_index].pane_label =
      header->AddChildView(+std::make_unique<views::Label>(pane_label_text));
  +panes_[pane_index].pane_label->SetEnabledColor(
      ui::kColorLabelForegroundSecondary);
  + + // Add provider dropdown
    +auto provider_model =
      std::make_unique<LlmProviderComboboxModel>(coordinator_);
  ...
  +panes_[pane_index].provider_selector->SetCallback(
      base::BindRepeating(+&ClashOfGptsView::OnProviderChanged,
                          base::Unretained(this), pane_index));
  +panes_[pane_index].provider_selector->SetAccessibleName(
      +u"LLM Provider Selection for Pane " +
      base::NumberToString16(pane_index + 1));
  + + // Add spacer
    +auto *spacer = header->AddChildView(std::make_unique<views::View>());
  +static_cast<views::BoxLayout *>(header->GetLayoutManager()) +
          ->SetFlexForView(spacer, 1);
  + + // Add open in new tab button
    +auto *open_button =
      header->AddChildView(+std::make_unique<views::ImageButton>(
          base::BindRepeating(+&ClashOfGptsView::OnOpenInNewTab,
                              base::Unretained(this), pane_index)));
  ...
  + + // Get WebContents from coordinator (it owns them)
    +content::WebContents *web_contents =
      coordinator_->GetOrCreateWebContentsForPane(pane_index);
  +if (web_contents) {
  ...
  + + // Enable focus and accelerators
    +panes_[pane_index].web_view->SetFocusBehavior(
...
+ + void ClashOfGptsView::OnProviderChanged(int pane_index) {
  ...
  + +auto selected_index =
      panes_[pane_index].provider_selector->GetSelectedIndex();
  +if (!selected_index ||
       selected_index.value() >= coordinator_->GetProviders().size()) {
    +return;
    +
  }
...
+ + void ClashOfGptsView::RecreatePanesContainer() {
  ...
  + + // Create panes
    ...
    + + // Add separator between panes (except after the last one)
      ...
      +auto *separator =
          panes_container_->AddChildView(+std::make_unique<views::Separator>());
...
+ + BEGIN_METADATA(ClashOfGptsView) + END_METADATA
