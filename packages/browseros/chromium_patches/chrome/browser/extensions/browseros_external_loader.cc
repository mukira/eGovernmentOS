diff-- git a / chrome / browser / extensions / browseros_external_loader.cc b /
        chrome / browser / extensions /
        browseros_external_loader
            .cc new file mode 100644 index 0000000000000..3bbb8c24621cc -- -/
        dev / null++ +
    b / chrome / browser / extensions / browseros_external_loader.cc @ @-0,
    0 + 1,
    709 @ @+ // Copyright 2024 The Chromium Authors
  ...
  + +namespace {
    + + // Interval for periodic maintenance
      +constexpr base::TimeDelta kPeriodicMaintenanceInterval =
        base::Minutes(15);
...
+        })");
    + + // Example JSON format:
      ...
      + //     "extension_id_1": {
      + //       "external_update_url":
        //       "https://example.com/extension1/updates.xml"
      + //     },
      + //     "extension_id_2": {
  ...
  + +void BrowserOSExternalLoader::ParseConfiguration(
    ...
    + + // Create the prefs dictionary in the format expected by
    ...
    + +for (const auto [extension_id, extension_config] : *extensions_dict) {
      ...
      + + // Copy supported fields
        ...
        +extension_prefs.Set(ExternalProviderImpl::kExternalUpdateUrl,
                             *update_url);
        +
      }
    ...
    + +LOG(INFO) << "Loaded " << prefs.size()
                 << " extensions from BrowserOS config";
    ...
    + + // Use a delayed task to ensure the extension system is fully
        // initialized
  ...
  + +void BrowserOSExternalLoader::PeriodicMaintenance() {
    ...
    + + // 1. Uninstall deprecated extensions (in kBrowserOSExtensions but not
        // in server config)
  ...
  + +void BrowserOSExternalLoader::ReinstallUninstalledExtensions() {
    ...
    + +for (const std::string &extension_id : browseros_extension_ids_) {
      ...
      + + // Find the extension's configuration
        +const base::Value::Dict *extension_config =
          last_config_.FindDict(extension_id);
      +if (!extension_config) {
      ...
      + + // Get the update URL from config
        +const std::string *update_url = +extension_config->FindString(
          ExternalProviderImpl::kExternalUpdateUrl);
      +if (!update_url) {
      ...
      + + // Add as pending extension for installation
        +pending_manager->AddFromExternalUpdateUrl(
            ...
            +Extension::WAS_INSTALLED_BY_DEFAULT, +false);
      ...
      +if (updater) {
        ...
        +LOG(INFO) << "browseros: Triggered immediate install for "
                   << extension_id;
        +
      }
      +
  ...
  + +void BrowserOSExternalLoader::OnConfigCheckComplete(
    ...
    +if (last_config_.empty()) {
      ...
      + // Compare with last config
        ...
        +const base::Value::Dict *old_config =
            last_config_.FindDict(extension_id);
        +if (!old_config || *old_config != new_config.GetDict()) {
          +config_changed = true;
          +LOG(INFO) << "browseros: Config changed for extension "
                     << extension_id;
          +break;
          +
        }
  ...
  + +void BrowserOSExternalLoader::TriggerImmediateInstallation() {
    ...
    + +LOG(INFO)
        << "browseros: Triggering immediate installation on first start";
    ...
    + + // Now trigger immediate high-priority installation
      +ExtensionUpdater *updater = ExtensionUpdater::Get(profile_);
    +if (!updater) {
      +LOG(WARNING) << "browseros: No extension updater available for "
                       "immediate installation";
      +return;
      +
    }
    + +LOG(INFO)
        << "browseros: Executing CheckNow with immediate install for " +
  ...
  + +void BrowserOSExternalLoader::LoadFromFile() {
    + // This runs on a background thread to avoid blocking the UI
        +base::ThreadPool::PostTaskAndReplyWithResult(
            ...
            +base::BindOnce(
                [](const base::FilePath &path) -> std::string {
                  +std::string contents;
                  +if (!base::ReadFileToString(path, &contents)) {
                    +LOG(ERROR)
                        << "Failed to read BrowserOS config file: " << path;
                    +return std::string();
                    +
                  }
                  +return contents;
  ...
  + +void BrowserOSExternalLoader::CheckAndLogExtensionState(
    ...
    + +for (const std::string &extension_id : browseros_extension_ids_) {
      ...
      + +if (registry->disabled_extensions().Contains(extension_id)) {
        ...
        +properties.Set("reason_reload", +disable_reasons.contains(
      ...
      + + // Log to metrics
        +browseros_metrics::BrowserOSMetrics::Log(
            "ota.extension.unexpected_state", +std::move(properties));
      + + // Also log to Chrome logs for local debugging
          +LOG(WARNING)
          << "browseros: Extension " << extension_id +
          << " in unexpected state: " << state + << " (context: " << context
          << ")";
      +
    }
    +
  ...
  + +void BrowserOSExternalLoader::UninstallDeprecatedExtensions() {
    ...
    + + // Check all BrowserOS-managed extensions
      ...
      + +LOG(INFO) << "browseros: Uninstalling deprecated extension "
                   << extension_id;
      + +std::u16string error;
      +if (!registrar->UninstallExtension(
               extension_id, +UNINSTALL_REASON_ORPHANED_EXTERNAL_EXTENSION,
               +&error)) {
        +LOG(WARNING)
            << "browseros: Failed to uninstall deprecated extension " +
            << extension_id << ": " << error;
        +
      }
      +
