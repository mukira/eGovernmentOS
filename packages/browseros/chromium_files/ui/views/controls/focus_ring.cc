...
SkColor GetPaintColor(FocusRing *focus_ring, bool valid) {
  ...
  // Check for explicit user color override
  if (auto color = focus_ring->GetColor(); color.has_value()) {
    return color.value();
  }
...
std::optional<ui::ColorId> FocusRing::GetColorId() const { return color_id_; }
...
// eGovernmentOS: Explicit color override implementation
std::optional<SkColor> FocusRing::GetColor() const {
  return color_user_override_;
}

void FocusRing::SetColor(SkColor color) {
  if (color_user_override_ == color) {
    return;
  }
  color_user_override_ = color;
  SchedulePaint();
}

float FocusRing::GetHaloThickness() const { return halo_thickness_; }

float FocusRing::GetHaloInset() const { return halo_inset_; }
...
bool FocusRing::ShouldPaintForTesting() { return ShouldPaint(); }
...
void FocusRing::OnViewFocused(View *view) { RefreshLayer(); }

void FocusRing::OnViewBlurred(View *view) { RefreshLayer(); }

void FocusRing::OnViewLayoutInvalidated(View *view) { InvalidateLayout(); }
