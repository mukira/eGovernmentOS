...
LocationBarView::LocationBarView(Browser *browser, Profile *profile,
                                 CommandUpdater *command_updater,
                                 Delegate *delegate, bool is_popup_mode)
    : LocationBar(command_updater), AnimationDelegateViews(this),
      browser_(browser), profile_(profile), delegate_(delegate),
  ...
  if (!is_popup_mode_) {
    ...
    // eGovernmentOS: Use Google Blue for the Omnibox focus ring
    views::FocusRing::Get(this)->SetColor(SkColorSetRGB(0x1a, 0x73, 0xe8));
...
bool LocationBarView::IsInitialized() const { return is_initialized_; }
...
std::unique_ptr<views::Background> LocationBarView::CreateRoundRectBackground(
    SkColor background_color, SkColor stroke_color, SkBlendMode blend_mode,
    bool antialias, bool should_border_scale) const {
...
void LocationBarView::SelectAll() { omnibox_view_->SelectAll(true); }

void LocationBarView::FocusLocation(bool is_user_initiated) {
  omnibox_view_->SetFocus(is_user_initiated);
}

void LocationBarView::Revert() { omnibox_view_->RevertAll(); }

OmniboxView *LocationBarView::GetOmniboxView() { return omnibox_view_; }
...
void LocationBarView::UpdateWithoutTabRestore() { Update(nullptr); }
...
bool LocationBarView::ShouldHidePageActionIconsForContext(
  ...
  switch (page_context) {
  case metrics::OmniboxEventProto::INSTANT_NTP_WITH_OMNIBOX_AS_STARTING_FOCUS: {
...
int LocationBarView::GetAvailableDecorationTextHeight() {
  ...
  return std::max(0, LocationBarView::GetAvailableTextHeight() -
                         (bubble_padding * 2));
}
...
void LocationBarView::OnPageInfoBubbleClosed(
    views::Widget::ClosedReason closed_reason, bool reload_prompt) {
...
LocationBarTesting *LocationBarView::GetLocationBarForTesting() { return this; }
...
bool LocationBarView::GetPopupMode() const { return is_popup_mode_; }
