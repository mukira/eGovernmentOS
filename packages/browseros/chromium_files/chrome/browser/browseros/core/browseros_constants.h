...
struct BrowserOSURLRoute {
  ...
  const char
      *extension_hash; // Hash/fragment without #, e.g., "ai" (empty if none)
};
...
// Fallback: If no exact hash match, falls back to route with empty hash for
// same page
...
inline const BrowserOSExtensionInfo *
FindBrowserOSExtensionInfo(const std::string &extension_id) {
